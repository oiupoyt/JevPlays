...
public class JevPlayerClientMod implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        instance = this;
        LOGGER.info("[{}] Initializing JevPlayer client mod v{} for Minecraft 1.21.11...",
        ...
        LOGGER.info("[{}] Initialized successfully! Provider: {}, Fair perception: {}",
    ...
    private void registerCommands() {
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
            dispatcher.register(ClientCommandManager.literal("jevplayer")
                    .then(ClientCommandManager.literal("status").executes(ctx -> {
                        ...
                        ctx.getSource().sendFeedback(Text.literal(statusMsg));
                        return 1;
                    }))
                    .then(ClientCommandManager.literal("start").executes(ctx -> {
                        engine.setEnabled(true);
                        ctx.getSource().sendFeedback(Text.literal("§a[JevPlayer] Autonomous play activated!"));
                        return 1;
                    }))
                    .then(ClientCommandManager.literal("stop").executes(ctx -> {
                        engine.setEnabled(false);
                        ctx.getSource().sendFeedback(Text.literal("§c[JevPlayer] Autonomous play stopped."));
                        return 1;
                    }))
                    .then(ClientCommandManager.literal("reload").executes(ctx -> {
                        loadConfig();
                        ctx.getSource().sendFeedback(Text.literal("§b[JevPlayer] Configuration reloaded."));
                        return 1;
                    }))
                    .then(ClientCommandManager.literal("test_gather_wood").executes(ctx -> {
                        ctx.getSource().sendFeedback(Text.literal("§e[JevPlayer] Testing Baritone wood gathering..."));
